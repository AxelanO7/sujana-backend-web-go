...
func AccountID(c *fiber.Ctx) int64 {
...
func verifyToken(tokenString string, secretKey []byte) (*domain.Account, error) {
	...
	if !token.Valid {
		return &domain.Account{}, fmt.Errorf("invalid token")
	}

	// Extract claims
	claims, ok := token.Claims.(jwt.MapClaims)
	if !ok {
		return &domain.Account{}, fmt.Errorf("invalid claims")
	}

	// Check if the token has an expiration claim
	expiration, ok := claims["exp"].(float64)
	if !ok {
		return &domain.Account{}, fmt.Errorf("expiration claim not found")
	}
	...
	if time.Now().After(expirationTime) {
		return &domain.Account{}, fmt.Errorf("token has expired")
	}

	// Extract user ID from the payload
	userClaims, ok := claims["user"].(map[string]interface{})
	if !ok {
		return &domain.Account{}, fmt.Errorf("user claim not found or not in the correct format")
	}

	id, ok := userClaims["id"].(float64) // Mengasumsikan bahwa ID adalah tipe data float64
	if !ok {
		return &domain.Account{}, fmt.Errorf("user id not found or not in the correct format")
	}

	accountRepo := repository.NewPostgreAccount(db.GormClient.DB)

	res, err := accountRepo.RetrieveAccountByID(uint(id))
	if err != nil {
		return nil, err
	}
